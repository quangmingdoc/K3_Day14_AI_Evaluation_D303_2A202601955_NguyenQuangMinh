...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            ...
            f"Answer: {answer}\n"
            f"Rubric:\n{rubric_text}\n"
            "Return a JSON object mapping each criterion to a score between 0 and 1."
        )
        ...
        try:
            ...
            if not scores:
                raise ValueError("No matching criteria in judge response")
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for entry in scores_batch:
            ...
            if scores:
                entry_averages.append(sum(scores.values()) / len(scores))
                all_scores.extend(scores.values())

        positional_bias = False
        if len(entry_averages) >= 2:
            first_half = entry_averages[0::2]
            second_half = entry_averages[1::2]
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        below_threshold = [name for name, score in scores.items() if score < 0.5]
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("hallucination"):
            suggestions.append(
                ...
                "and tighten faithfulness guardrails"
            )
        ...
        if categories.get("off_topic"):
            suggestions.append(
                "Review intent detection and routing logic to reduce off-topic responses"
            )

        default_suggestions = [
            "Increase chunk size in the RAG pipeline to reduce context fragmentation",
            "Expand the golden dataset to cover the observed failure patterns",
            "Add human review for low-confidence responses before deployment",
        ]
